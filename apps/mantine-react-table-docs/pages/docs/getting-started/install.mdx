...
> This is the install guide for `mantine-react-table` **V8** and `@mantine/` packages **V8**. MRT V8 will not work with Mantine V6.
...
> `mantine-react-table` V8 requires **Mantine V8.0.0+** packages as dependencies in your project.
>
...
import '@uponu-solutions/mantine-react-table/styles.css'; //import MRT styles
```

### Common Errors

If you don't see any styles applied to the mantine table components, you may have mismatching versions of `mantine-react-table` and `@mantine/core`. MRT v1 requires Mantine v6 and will not work with Mantine v8. MRT v8 will be released by the end of 2023 and will work with Mantine v8.
